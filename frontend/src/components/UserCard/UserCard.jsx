...
export const UserCard = ({ user }) => {
  const { followUser, getFollowingsUserAuth } = useFollowersActions();
  const userAuth = useSelector((state) => state.users.auth.user);
  const { followingsUserAuth, status: statusFollow } = useSelector(
  ...
  const isFollowing = () => {
    return followingsUserAuth.some((follow) => follow.id == user.id);
  };
